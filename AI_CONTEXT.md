...
- Version : 1.7.0. Licence MIT. Auteur : Teo Comyn. Dépôt : https://github.com/teocomyn/t4c2.dev
- Décisions : mots-clés insensibles à la casse ; noms sensibles à la casse ; listes indexées à 1 ; préfixe ; `( )` seulement pour grouper ; `fiche` pour les données plates ; `modele` / `nouveau` / `sur` pour les objets ; `nuple` immuable ; `ensemble` sans doublon ; `importe` pour maths/temps/fichiers ; pas de réseau dans le playground.
